function [] = IED_CCG(window,Bin_ms)
...
load('NY609_Day4_Part2_IED.mat'); 
IED_new = IED(~cellfun('isempty',IED));  
res_file = dir('*Ch1IED*');
load(res_file.name);
load('regionCH.mat');
load('goodCH.mat');
Dev_ECoG_map = 1:length(IED_new);
dimensions = [8 length(IED_new)./8];
Rs = 1250; % sampling rate
Wn = window*2; % Wn = window 

refied_time = IED_times; 
...
for j = 1:length(IED_new)
    res_chan = IED_new{j,1}.*Rs;
