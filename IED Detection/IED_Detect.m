function IED = IED_Detect(num_CH, IED_CH, varargin)
% IED_DETECT Detects IED events in LFP file
...
elseif res == 0
   ...
   IED = zeros(1,3); 
    ...
    [IED] = IED_sep(large_IEDindex, :);
     
end

% Step 2: Remove peaks too close together using inter-IED interval
for ii = 1:length(IED(:, 2))-1
    if  IED(ii+1, 2) - IED(ii, 2) < ied_int
        IED(ii+1, 2) = (IED(ii+1, 2) + IED(ii, 2))/2;
        IED(ii, :) = 0;
    end
end
...
% (Optional argument required in specific datasets)
% Step 3: Remove bursts too close together using minburst interval 
%
% diff_IED = diff(IED(:, 1));
% sep_IEDs = find(diff_IED < minburstInt);
% sep_IEDs = sep_IEDs + 1;
% IED(sep_IEDs, :) = [];

% Step 4: Remove noisy peaks based on amplitude of raw signal
if isempty(IED)==1 
    disp('No IED Events');
else
    noise_removal; 
    ...
    large_file = strcat(fbasename,'_Ch', num2str(IED_CH), '_IED');
    save(large_file,'IED');
    IEDs_name = strcat(num2str(IED_CH),'.ied.evt');
    channelID = IED_CH - 1;

    IEDevts = [IED(:, 1)-0.0625 IED(:, 1) IED(:, 1)+0.0625];
